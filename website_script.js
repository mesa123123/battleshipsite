...
function fireRound(){
  ...
  if( playerAlive == true && (isNaN(selection) || grid[selection] == -1 || selection < 0 || selection > 6) ){
    ...
    if(grid[selection] == 0){
      document.getElementById("Result").innerHTML = "Intel: It appears you missed, enemy ship still active";
      grid[selection] = -1;
    } else{
    ...
    if(i > 0 && shipAlive == true){
      ...
      switch(i){
        case 3:
            document.getElementById("intelConsole").innerHTML = "Intel: Attained visuals on ship;  Where should we fire?";
          break;
        case 2:
            document.getElementById("intelConsole").innerHTML = "Intel: Enemy Ship Loading Ammunition; Where should we fire?";
          break;
        case 1:
            document.getElementById("intelConsole").innerHTML = "Intel: Enemy Ship Taking Aim; Where should we fire?";
          break;
        default:
            document.getElementById("intelConsole").innerHTML = "Intel: Attained visuals on ship;  Where should we fire?";
        //Check the selection is valid and the User isn't bombing the same place
...
function result(){
  ...
  //Lose
  if( shipAlive== true ){
    document.getElementById("intelConsole").innerHTML = ("Intel: It appears as though we gave them enough time to fire... \
